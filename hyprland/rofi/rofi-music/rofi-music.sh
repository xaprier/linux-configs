...
if ! pgrep -x spotify >/dev/null; then
    bash -c 'LD_PRELOAD=/usr/local/lib/spotify-adblock.so /usr/bin/spotify' &
    exit
fi
...
if [ "$(/usr/local/bin/spotifycli --playbackstatus)" = "▶" ]; then
    play_pause='▮▮ Pause'
else
    play_pause=' Play'
fi
...
current="$(/usr/local/bin/spotifycli --status)"
...
/usr/local/bin/spotifycli "$action"
